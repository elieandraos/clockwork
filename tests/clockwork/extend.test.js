...
test("it can validate a custom rule", () => {
    ...
    clockwork
        .setRules(rules)
        .setData(data)
        .setCustomErrorMessages({ 'age.greater_than_ten': 'age should be greater than 10!' })
...
test("it can validate a custom rule", () => {
    ...
    clockwork
        .setRules(rules)
        .setData(data)
        .setCustomErrorMessages({ 'age.greater_than': 'age should be greater than {param}!' })
...
test("it throws an error if the name and closure arguments are not provided", () => {
    let withoutName = () => { clockwork.extend(null, null) };
    expect(withoutName).toThrow(Error);
});

test("it throws an error if the rule already exist", () => {
    let ruleExists = () => { clockwork.extend('starts_with', () => { return true }) };
